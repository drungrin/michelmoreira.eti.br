...
// The no-trailing-slash contract (Pitfall 1 / A). Every internal link, the
// language switcher, and later hreflang/sitemap route through localeUrl().
// Locked here so the canonical `/en` (no trailing slash) can never drift.
...
test('localeUrl: no return value ends in a trailing slash except the bare pt root', () => {
  ...
  for (const [locale, path] of cases) {
    const result = path === undefined ? localeUrl(locale) : localeUrl(locale, path);
